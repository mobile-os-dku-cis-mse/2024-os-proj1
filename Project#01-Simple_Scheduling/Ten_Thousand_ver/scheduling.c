        ...
        if(current_ready != NULL){
            ...
            if(current_ready->pcb.cpu_burst <= 0){
                if(current_ready->pcb.io_burst <= 0){
                    ...
                    enqueue(ready_queue, current_ready->pcb.pid, current_ready->pcb.original_cpu_burst, current_ready->pcb.original_io_burst, current_ready->pcb.original_cpu_burst, current_ready->pcb.original_io_burst, current_ready->pcb.arrival_time, time_quantum);
                    dequeue(ready_queue);
                }
                else{
                    log_process_event(current_ready, "Moved to WAIT queue for I/O");
                    enqueue(wait_queue, current_ready->pcb.pid, current_ready->pcb.original_cpu_burst, current_ready->pcb.original_io_burst, 0, current_ready->pcb.io_burst, current_ready->pcb.arrival_time, time_quantum);
                    dequeue(ready_queue);
                }
            }        
            else{
        ...
        if(current_wait != NULL){
            if(current_wait->pcb.io_burst <= 0){
                if(current_wait->pcb.cpu_burst <= 0){
                    ...
                    log_process_event(current_wait, "Re-enqueued to Ready queue");
                    current_wait->pcb.flag = 0;
                    current_wait->pcb.arrival_time = time_count;

                    enqueue(ready_queue, current_wait->pcb.pid, current_wait->pcb.original_cpu_burst, current_wait->pcb.original_io_burst, current_wait->pcb.original_cpu_burst, current_wait->pcb.original_io_burst, current_wait->pcb.arrival_time, time_quantum);
                    dequeue(wait_queue);
                }
            }   
            else{
        ...
        if (current_ready != NULL) {
            ...
            if (current_ready->pcb.remaining_time == 0) {
                if (current_ready->pcb.cpu_burst <= 0) {
                    ...
                    log_process_event(current_ready, "Re-enqueued to Ready queue");
                    enqueue(ready_queue, current_ready->pcb.pid, current_ready->pcb.original_cpu_burst, current_ready->pcb.original_io_burst, current_ready->pcb.cpu_burst, current_ready->pcb.io_burst, current_ready->pcb.arrival_time, time_quantum);
                    dequeue(ready_queue);
                }
            }
        ...
        if (current_wait != NULL) {
            if(current_wait->pcb.io_burst <= 0){
                if(current_wait->pcb.cpu_burst <= 0){
                    ...
                    enqueue(ready_queue, current_wait->pcb.pid, current_wait->pcb.original_cpu_burst, current_wait->pcb.original_io_burst, current_wait->pcb.cpu_burst, 0, current_wait->pcb.arrival_time, time_quantum);
                    dequeue(wait_queue);
                }
            }
            else{
